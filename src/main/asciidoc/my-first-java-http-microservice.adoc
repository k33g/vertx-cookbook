...
==== Disvover services and call them

For each service found, we'll call the service. For each record, we'll create a `ServiceReference` from the current record. Then we'll be able to instantiate a `WebClient` from the reference. And then it will be possible to call the service from the client (`client.get(uri).send()`).

So, update the discovery part of the source code like that:
  ...
  .subscribe(
    successfulRecords -> {
      ...
      successfulRecords.forEach(record -> {
        ...
        String uri = record.getLocation().getString("root")+"/"+record.getMetadata().getString("uri");
        // eg: uri = "/root/hello"
        client.get(uri).send(
          resp -> {
...
🌍 Listening on 8081
😢 no record found
😃 Microservice is published! e89fbc50-c5bb-4fe8-81ca-10de16d263f8
```
...
🌍 Listening on 8082
😋 1 record(s) found.
😃 Microservice is published! 1a8729b2-ad5b-4ae4-857d-6d9a990383da
{"message":"👋 Hello 🌍"}
```

Again, open an other terminal (the last one):

```
SERVICE_NAME="YO-SERVICE" SERVICE_PORT=8083 PORT=8083 java -jar target/myservice-1.0-SNAPSHOT-fat.jar 
```
...
🌍 Listening on 8083
😋 2 record(s) found.
😃 Microservice is published! c1b31fba-306d-45b8-802b-577b391cfbb3
{"message":"👋 Hello 🌍"}
{"message":"👋 Hello 🌍"}
```
